...
bool Utility::u2ip(const std::string& host, struct sockaddr_in& sa, int ai_flags)
{
	...
	{		
		...
		split(host,".",tokens);
		u.a.b1 = static_cast<unsigned char>(atoi(tokens[0].c_str()));
		u.a.b2 = static_cast<unsigned char>(atoi(tokens[1].c_str()));
		u.a.b3 = static_cast<unsigned char>(atoi(tokens[2].c_str()));
		u.a.b4 = static_cast<unsigned char>(atoi(tokens[3].c_str()));
...
void Utility::split(std::string s,std::string delimiter,std::vector<std::string>& outs)
{	
	outs.clear();
